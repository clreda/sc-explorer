...
sidebar <- dashboardSidebar(
  ...
  sidebarMenu(id = "sidebarMenu",
	      ...
	      menuItem("GE clustering", tabName="clustering", icon=icon('cog'))
	)
  )
...
body <- dashboardBody(
                       ...
                       box(title = "Choice of gene expression level threshold", width = 12, solidHeader = TRUE, 
                           ...
                           fluidRow(
                             ...
                             column(width = 3, 
                                    ...
                                    p(class = "text-muted", 
					"The threshold is set in percentage of gene expression respect to the total gene expression in each cell.")
                                    ),
  ...
  ## Clustering tab ##
    tabItem("clustering",
            fluidRow(
              box(title = "Options for clustering", 
                  width = 3, solidHeader = TRUE, 
                  status = "info", height = "500px",
                  br(),
                  sliderInput("thresholdClustering", "Threshold Value:", 0.5, 1, 0.75, step=1e-2),
                  sliderInput("thresholdNeighborsClustering", "Threshold (for Neighbors) Value:", 0.5, 1, 0.75, step=1e-2),
	          sliderInput("featureSelectionClustering", "Feature Correlation Threshold Value:", 0.5, 1, 0.9, step=1e-2),
                  sliderInput("frequencyClustering", "Feature Frequency Threshold Value:", 0.5, 1, 0.7, step=1e-2)#,
		  #uiOutput("runClustering"),
                  #column(width = 4, uiOutput("replicates")) in utils.Rdata
		),
            box(title = "Clustering using MineClusters for selected features, samples and options", 
                width = 9, solidHeader = TRUE, 
                status = "primary", height = "500px",
		br()
                #uiOutput("messagegeclustering"),
                #plotOutput("plotClustering")
		)
            )
    ),
